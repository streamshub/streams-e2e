...
/**
 * Installer apicurio operator using olm from catalog
 */
public class ApicurioOlmCatalogInstaller {
    private static final Logger LOGGER = LoggerFactory.getLogger(ApicurioOlmCatalogInstaller.class);

    private static final String SUBSCRIPTION_NAME = "apicurio";

    /**
     * Install apicurio operator from catalog presented on cluster using OLM
     *
    ...
    private static boolean isOperatorReady(String ns) {
        try {
            PodUtils.waitForPodsReadyWithRestart(ns, new LabelSelectorBuilder()
                .withMatchLabels(Map.of("name", "apicurio-registry-operator")).build(), 1, true);
            LOGGER.info("Apicurio operator in namespace {} is ready", ns);
            return true;
        } catch (Exception ex) {
            return false;
        }
